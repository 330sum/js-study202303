...
function runUpdownGame(gameData) {
  ...
  // gameData 디스트럭처링(구조분해할당)
  const { countDown, secret, answer} = gameData
  // 디스트럭처링은 사본을 만드는 것이기 때문에, 사본을 변경해도 원본에 적용되지 않음!
  // 그래서 76번 라인은 꼭 gameData.countDown--; 써야함! (원본수정이 필요한 것을 구조분해할당 하면 안됨)
  // 구조분해할당을 하면 지저분한 코드를 개선할 수 있음

  while (true) {
        
    if (!inputAnswerAndValidate(gameData)) continue;

    gameData.countDown--; //원본을 수정해야 하기 때문에 그대로사용해야함

    if (secret === answer) {
      alert(`정답입니다! ${initCount - countDown}번만에 맞췄습니다!`);
      gameData.gameEndFlag = true; //원본을 수정해야 하기 때문에 그대로사용해야함
      break;
    } else if (secret > answer) {
      alert(`UP!!`);
      gameData.min = answer + 1; // min도 원본 수정!
    } else {
      alert(`DOWN!!`);
      gameData.max = answer - 1; // max도 원본 수정!
    }
